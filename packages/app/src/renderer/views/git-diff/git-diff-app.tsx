...
/**
 * Embed view that renders a diff for a single file. Receives
 * `{ directory, path }` via view args (forwarded by the main shell
 * when it catches `openDiffInActivePane`), then fetches live
 * `pr.getStatus` to find the matching `GitFileStatus` entry so the
 * existing `<DiffViewer>` (the same one the full Git view uses) can
 * do the actual rendering.
 *
 * We poll status on the same cadence as `git-client.tsx` so the diff
 * refreshes when the file changes on disk, and the view shows a
 * graceful placeholder if the file leaves the worktree (committed,
 * discarded, etc).
 */
export function GitDiffApp() {
  useThemeSync()
  const args = useViewArgs<GitDiffArgs>()
  ...
  return <GitDiffPane key={`${directory}::${path}`} directory={directory} path={path} />
}
