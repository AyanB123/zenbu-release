 ...
 * panel. Closely mirrors VS Code's Terminal-view tab UX: a tinted
 * column on `bg-muted/40`, a compact header at the top with
...
export function TerminalTabs({
  ...
  return (
    <div
      className="flex h-full w-full flex-col overflow-hidden border-b border-r bg-muted/40"
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // 11px text, tight horizontal padding). `min-h` (not `h`)
        // so the row can grow vertically when the title wraps to
        // a second line.
        "group flex min-h-[22px] select-none items-center gap-[6px] px-2 py-[3px] text-[11px] leading-[1.25]",
        isActive
          ? "bg-sidebar-accent text-sidebar-accent-foreground"
          : "text-muted-foreground hover:bg-sidebar-accent/40 hover:text-foreground",
      )}
    >
      {/* No `truncate` here — the strip is narrow so titles will
        * commonly overflow, but the user explicitly wants to see
        * the whole thing rather than `gui-terminal-..`. We keep
        * `min-w-0` so flexbox still lets the trash button reserve
        * its slot, and use `break-all` so the wrap happens between
        * any two characters (filenames don't have spaces). The row
        * is `min-h-[22px]` rather than fixed-height now so it can
        * grow vertically when wrapped onto multiple lines. */}
      <span
        aria-label={title}
        title={title}
      ...
      <button
        ...
        className={cn(
          ...
          // Always visible on the active row so you can dismiss the
          // currently-focused tab without first hovering away from it.
          // Inactive rows reveal it on hover.
