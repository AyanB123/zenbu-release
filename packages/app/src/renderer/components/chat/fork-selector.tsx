 ...
 *     own fork UI: bigger text, each row is "<message text>" with
 *     a "Message N of M" subline.
 *
...
type Row = {
  ...
  label: string
  /** 1-based position among user messages in this session. Surfaces
   * as "Message N of M" so the user can locate a turn by ordinal
   * even when the search query has filtered the list. */
  position: number
}
...
export function ForkSelector({
  ...
  // Total user messages in the session (denominator of "N of M").
  // Filter happens AFTER counting so a search doesn't shrink the
  // total — only the visible rows shrink.
  const allUserRows = useMemo<Row[]>(() => {
    ...
    for (const e of entries) {
      if (e.kind === "message" && e.messageRole === "user") {
        pos += 1
        out.push({ id: e.id, label: e.label, position: pos })
      }
    }
    return out
  }, [entries])
  ...
  return (
    ...
    <div className="mx-auto w-full max-w-[919px] px-2 pt-1 pb-2">
      ...
      >
        <div className="shrink-0 border-b border-border bg-muted/40 px-3 py-1.5">
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span className="text-[10px] tabular-nums text-muted-foreground">
              ...
              {searchQuery ? ` / ${allUserRows.length.toLocaleString()}` : ""}
            </span>
          </div>
        ...
        <div className="shrink-0 border-t border-border bg-muted/30 px-3 py-1 text-[10px] tabular-nums text-muted-foreground">
          {busy ? (
            <span className="text-shimmer">Forking…</span>
          ) : (
            <>
              ({filteredRows.length === 0 ? 0 : selectedIndex + 1}/
              {filteredRows.length})
            </>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-[13px] leading-[1.3]">{row.label}</div>
        <div
          className={cn(
