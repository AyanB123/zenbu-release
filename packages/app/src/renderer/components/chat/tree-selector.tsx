...
function BrowseView({
  ...
  return (
    <>
      ...
      <div
        ref={listRef}
        className="min-h-0 flex-1 overflow-auto px-1 py-1 font-mono text-[11px] leading-[1.35]"
      >
        {loading ? (
    ...
    <div className="shrink-0 border-b border-border bg-muted/40 px-3 py-1.5">
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="text-[10px] tabular-nums text-muted-foreground">
          ...
          {showAll ? "" : ` / ${totalAll.toLocaleString()}`}
        </span>
      </div>
      <div className="mt-0.5 truncate text-[10px] text-muted-foreground">
        ↑↓/jk: move &nbsp; ←→/hl: page &nbsp; ⌃u/⌃d: half-page &nbsp;
        g/G: top/bottom &nbsp; ⏎: select &nbsp; ⌃a: {showAll ? "default" : "show all"} &nbsp; esc: close
      </div>
