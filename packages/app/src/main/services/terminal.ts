...
export class TerminalService extends Service.create({
  ...
  deps: {
    ...
    // server to already be live before the iframe can resolve
    // `/views/terminal/index.html` through it.
  ...
  evaluate() {
    ...
    // Register the terminal as a bottom-panel view. The host shell
    // discovers it via `meta.bottomPanel === true`, so additional
    // plugins can contribute their own bottom-panel views the same
    // way they contribute right-sidebar views today.
    this.setup("register-view", () => {
      this.ctx.viewRegistry.registerAlias({
        type: "terminal",
        reloaderId: "app",
        pathPrefix: "/views/terminal",
        meta: { kind: "view", bottomPanel: true, label: "Terminal" },
      })
      return () => {
        void this.ctx.viewRegistry.unregister("terminal")
      }
    })
