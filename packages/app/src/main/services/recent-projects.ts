...
export class RecentProjectsService extends Service.create({
  ...
  async evaluate() {
    // Run once on boot. Cheap (a few file reads + one short
    // `sqlite3` invocation per IDE) and we *want* the renderer's
    // onboarding view to have the list ready before the user
    // even sees it.
