    ...
    >
      ...
      {isPiExtension && match ? (
        ...
        (listing?.readme != null || match) && (
          <ReadmeBody readme={listing?.readme ?? readme} error={readmeError} />
        )
      )}
    </DetailLayout>
  );
...
function PiExtensionBody({ dir }: { dir: string }) {
  return (
    <div className="text-[13px] text-muted-foreground">
      Loaded from{" "}
      <code className="rounded bg-muted px-1 py-0.5 text-[12px]">{dir}</code>.
    </div>
  );
}
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col bg-background text-foreground">
      <div className="shrink-0 border-b border-border/60">
        <div className="mx-auto w-full max-w-[820px] px-4 py-4">{header}</div>
      </div>
