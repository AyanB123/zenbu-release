...
type PlayConfig = InferSchemaRoot<typeof playSchema>["configs"][string]

const here = path.dirname(fileURLToPath(import.meta.url))
const VIEW_SOURCE = path.resolve(here, "../../views/play-button-view.tsx")
const VIEW_TYPE = "play-button"
...
export class PlayService extends Service.create({
  ...
  async evaluate() {
    // Title-bar component view registration lives alongside the
    // service so the plugin has one main-process entry point.
  ...
  private async resetStaleRunFlags(): Promise<void> {
    ...
    const dirty: string[] = []
    for (const [id, cfg] of Object.entries(root.play.configs)) {
    ...
    await this.ctx.db.client.update(root => {
      for (const id of dirty) {
        const cfg = root.play.configs[id]
        if (!cfg) continue
        cfg.isRunning = false
  ...
  async saveConfig(args: {
    ...
    await this.ctx.db.client.update(root => {
      const existing = root.play.configs[args.workspaceId]
      if (existing) {
        ...
        root.play.configs[args.workspaceId] = {
  ...
  async run(args: {
    ...
    const root = this.ctx.db.client.readRoot()
    const cfg = root.play.configs[args.workspaceId]
    if (!cfg) {
    ...
    await this.ctx.db.client.update(root => {
      const c = root.play.configs[args.workspaceId]
      if (!c) return
      c.isRunning = true
  ...
  private attachChild(entry: LiveRun, env: NodeJS.ProcessEnv): void {
    ...
    entry.child.on("error", err => {
      ...
      this.appendSystem(entry, `\n[error] ${msg}\n`)
      this.ctx.rpc.emit.play.playExit({
    ...
    entry.child.on("exit", code => {
      if (entry.phase === "setup" && code === 0) {
        ...
        void this.ctx.db.client.update(root => {
          const cfg = root.play.configs[entry.workspaceId]
          if (!cfg) return
  ...
  private appendLog(
    ...
    this.ctx.rpc.emit.play.playLog({
      workspaceId: entry.workspaceId,
      runId: entry.runId,
      stream,
      data,
  ...
  private flush(entry: LiveRun): void {
    ...
    entry.pendingLogs = []
    void this.ctx.db.client.play.configs[entry.workspaceId]?.logs
      ?.concat(batch)
      .catch(err => {
  ...
  private async flushAsync(entry: LiveRun): Promise<void> {
    ...
    entry.pendingLogs = []
    try {
      await this.ctx.db.client.play.configs[entry.workspaceId]?.logs?.concat(
        batch,
      )
    } catch (err) {
      console.error("[play] logs.concat failed:", err)
